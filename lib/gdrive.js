...
class GoogleDriveSynchronizedFile {
    ...
    async sync () {
        ...
        if (local && !remote) {
            ...
            log('Remote file does not exist yet. Attempt to upload it.');
            await this._createRemoteFile();
        } else if (!local && remote) {
            log('Local file does not exist yet. Attempt to download it.');
    ...
    async _downloadFile (remoteFile) {
        ...
        try {
            await pipeline(createReadStream(tempPath), createWriteStream(this.localFilePath));
        } catch (err) {
            log(`Error copying from ${tempPath} to ${this.localFilePath}: ${err.stack}`);
        }

        await utimes(this._localFilepath, remoteFile.mtime, remoteFile.mtime);
    }
    ...
    async _createRemoteSubdirs () {
        ...
        for (let i = 0; i < subdirs.length; i++) {
            ...
            if (!dirId) {
                log(`Create partial directory ${subdir}`);
